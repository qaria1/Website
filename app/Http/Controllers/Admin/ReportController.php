...
class ReportController extends Controller
{
    public function admin_earning(Request $request)
    {
        ...
        $digital_payment_query = Order::where(['order_status' => 'delivered'])->whereNotIn('payment_method', ['cash', 'pay_on_delivery', 'pay_by_wallet', 'offline_payment']);
        $digital_payment = self::earning_common_query($request, $digital_payment_query)->sum('order_amount');

        $cash_payment_query = Order::where(['order_status' => 'delivered'])->whereIn('payment_method', ['cash', 'pay_on_delivery']);
        ...
        $admin_subscription_earn = $filter_data['subscription'];
        ...
        foreach($inhouse_earn as $key=>$earning) {
            ...
            $total_subscription += $admin_subscription_earn[$key];
            ...
            $total_earning_statistics[$key] = ($earning+$admin_commission_earn[$key]+$admin_subscription_earn[$key]+$shipping_earn[$key])-$discount_given[$key]-$refund_given[$key];
        }
        ...
        return view('admin-views.report.admin-earning', compact('earning_data', 'inhouse_earn', 'shipping_earn',
            'admin_commission_earn', 'admin_subscription_earn', 'refund_given', 'discount_given', 'total_tax', 'from', 'to', 'date_type', 'payment_data'));
    }
    ...
    public function admin_earning_excel_export(Request $request){
        ...
        foreach ($inhouse_earn as $key=>$earning) {
            ...
            $data[] = array(
                ...
                'Subscription Earning' => BackEndHelper::set_symbol(BackEndHelper::usd_to_currency($admin_subscription_earn[$key])),
    ...
    public function earning_same_month($type, $start_date, $end_date, $month_date, $number, $default_inc){
        ...
        for ($inc = $default_inc; $inc <= $number; $inc++) {
            $day = date('jS', strtotime("$year_month-$inc"));
            $subscription[$day.'-'.$month] = 0;
            foreach ($subscriptions as $match) {
                if ($match['day'] == $inc) {
                    $subscription[$day.'-'.$month] = $match['amount'];
                }
            }
        }
    ...
    public function earning_this_week($type){
        ...
        for ($inc = 0; $inc <= $number; $inc++) {
            $subscription[$day_name[$inc]] = 0;
            foreach ($subscriptions as $match) {
                if ($match['day'] == $day_name[$inc]) {
                    $subscription[$day_name[$inc]] = $match['amount'];
                }
            }
        }
    ...
    public function earning_same_year($type, $start_date, $end_date, $from_year, $number, $default_inc){
        ...
        for ($inc = $default_inc; $inc <= $number; $inc++) {
            $month = date("F", strtotime("2023-$inc-01"));
            $subscription[$month.'-'.$from_year] = 0;
            foreach ($subscriptions as $match) {
                if ($match['month'] == $inc) {
                    $subscription[$month.'-'.$from_year] = $match['amount'];
                }
            }
        }
