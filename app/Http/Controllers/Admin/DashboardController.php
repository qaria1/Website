...
class DashboardController extends BaseController
{
    ...
    public function dashboard(): View
    {
        ...
        for ($i = 1; $i <= 12; $i++) {
            $subscriptionEarningStatisticsData[$i] = SubscriptionTransaction::whereMonth('created_at', $i)->whereYear('created_at', now()->format('Y'))->sum('paid_amount');
        }
    ...
    public function getEarningStatistics(Request $request): JsonResponse
    {
        ...
        $subscriptionEarningStatisticsData = [];
        if ($dateType == 'yearEarn') {
            $subscriptionLabel = array("Jan", "Feb", "Mar", "April", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec");
            // $subscriptionEarningStatisticsData = SubscriptionTransaction::whereBetween('created_at', [Carbon::now()->startOfYear()->format('Y-m-d'), Carbon::now()->endOfYear()->format('Y-m-d')])
            // ->groupby(['year', 'month'])
            // ->get()->toArray()
            // ->sum('paid_amount');

            $subscriptionEarningStatisticsData = $this->getSubscriptionEarningStatisticsData(
                dataRange: ['from' => Carbon::now()->startOfYear()->format('Y-m-d'), 'to' => Carbon::now()->endOfYear()->format('Y-m-d')],
                groupBy: ['year', 'month'],
            );
            ...
            $to = date('Y-m-t');
            $number = date('d', strtotime($to));
            $subscriptionLabel = range(1, $number);
            // $subscriptionEarningStatisticsData = SubscriptionTransaction::
            ...
            $from = Carbon::now()->startOfWeek()->format('Y-m-d');
            $to = Carbon::now()->endOfWeek()->format('Y-m-d');
            $subscriptionLabel = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
            // $subscriptionEarningStatisticsData = SubscriptionTransaction::whereMonth('created_at', $i)->whereYear('created_at', now()->format('Y'))->sum('paid_amount');
        ...
        $data = [
            ...
            'subscription_earn' => array_values($subscriptionEarningStatisticsData)
        ];

        return response()->json($data);
    }

    public function getSubscriptionEarningStatisticsData(string $sellerIs = null, array $dataRange = [], array $groupBy = [], int $dateStart = 1, int $dateEnd = 12): array
    {
        $subscriptionEarningStatisticsData = [];
        $subscriptionEarnings = SubscriptionTransaction::
            when($dataRange, function ($query) use ($dataRange) {
                return $query->whereBetween('created_at', [$dataRange['from'], $dataRange['to']]);
            })
            ->select(
                DB::raw('IFNULL(sum(paid_amount),0) as sums'),
        ...
        if (in_array('day', $groupBy)) {
            for ($inc = $dateStart; $inc <= $dateEnd; $inc++) {
                $subscriptionEarningStatisticsData[$inc] = 0;
                foreach ($subscriptionEarnings as $match) {
                    if ($match['day'] == $inc) {
                        $subscriptionEarningStatisticsData[$inc] = $match['sums'];
                    }
                }
            }
        } else {
        ...
        return $subscriptionEarningStatisticsData;
    }
