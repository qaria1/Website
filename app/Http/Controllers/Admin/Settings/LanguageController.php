...
class LanguageController extends BaseController
{
    ...
    public function updateTranslate(Request $request, $lang): void
    {
        ...
        $dataFiltered[$request['key']] = $request['value'];
    ...
    // Automation on language translation
    public function autoTranslateAll(Request $request): JsonResponse
    {

        $langCode = getLanguageCode($request['lang']);

        $fullData = include(base_path('resources/lang/' . $request['lang'] . '/messages.php'));
        $dataFiltered = [];
        ...
        // $array = include base_path('resources/lang/en/messages.php');

        foreach ($dataFiltered as $key => $value) {

            $translated = autoTranslator($value, 'en', $langCode);
            $dataFiltered[$key] = $translated;

            $string = "<?php return " . var_export($dataFiltered, true) . ";";
            file_put_contents(base_path('resources/lang/' . $request['lang'] . '/messages.php'), $string);
