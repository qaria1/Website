...
use App\Exports\SellerSignUpListExport;
...
class VendorController extends BaseController
{
    ...
    public function getAddView(Request $request): View
    {
        ...
        if($getAllSellers->count() > 0){
            foreach($getAllSellers as $seller){
                array_push($assignedBrands, $seller->brand_id);
            }
        }

        array_push($assignedBrands, 1);

        $assignedBrands = array_unique($assignedBrands);
    ...
    {
        $this->vendorRepo->update(id: $request['id'], data: ['status' => $request['status'], 'suspend_reason' => $request['suspend_reason'] ?? null, 'reject_reason' => $request['reject_reason'] ?? null]);
    ...
    public function getView(Request $request, $id, $tab = null): View|RedirectResponse
    {
        ...
        if ($tab == 'order') {
            ...
            return $this->getSubscriptionListTabView(request: $request, seller: $seller);
        }   else if ($tab == 'review') {
    ...
    public function getSubscriptionListTabView(Request $request, $seller): View
    {
        ...
        if($getAllSellers->count() > 0){
            foreach($getAllSellers as $vendor){
                array_push($assignedBrands, $vendor->brand_id);
            }
        }
    ...
    public function signUpRequest(Request $request): View
    {
        $subscriptionPlans = SubscriptionPlan::all();
        $current_date = date('Y-m-d');
        ...
        Seller::where(['status' => 'pending', 'checked' => 0])->update([
            'checked' => true,
        ]);

        $request->flash();

        return view('admin-views.seller.sign-up-requests', compact('sellers', 'current_date', 'subscriptionPlans'));
    }

    public function exportSignUpRequestList(Request $request): BinaryFileResponse
    {
        $filters = [
            'status' => ['pending'],
        ];
        ...
        $active = $sellers->where('status', 'pending')->count();
        $inactive = $sellers->where('status', '!=', 'pending')->count();
        $data = [
            'sellers' => $sellers,
            'search' => $request['searchValue'],
            'active' => $active,
