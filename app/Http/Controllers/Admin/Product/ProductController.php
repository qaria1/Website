...
class ProductController extends BaseController
{
    ...
    public function add(ProductAddRequest $request, ProductService $service): JsonResponse|RedirectResponse
    {
        ...
        // check if who adds the product so that to consider the Subscription Plan Code
        // if admin custom plan code like X, if seller get plan code from logged in seller
        ...
        // $subCategoryCode = intval($dataArray['sub_category_id']);
        // dd($categoryCode);
        ...
        */
        ...
        // $subCategoryCode = (Category::findOrFail($subCategoryCode))->category_code;
        ...
        // $deliveryClassCode = Category::where('id', intval($dataArray['sub_category_id']))->first()?->deliveryClass?->code;


        // Get the last product_sub_code with the given category and subcategory codes
        $lastProductSubCode = ProductModel::where('product_sub_code', 'like', $categoryCode . $subCategoryCode . $deliveryClassCode . '%')
        ...
        $productSubCode = 'M-' . $categoryCode . $subCategoryCode . $deliveryClassCode .  $newProductSubCodeNumberPadded;
        // dd($productSubCode);
    ...
    public function getUpdateView(string|int $id): View
    {
        ...
        $seller = \App\Models\Seller::where('id', $product?->user_id)->first();
