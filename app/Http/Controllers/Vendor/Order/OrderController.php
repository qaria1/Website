...
class OrderController extends BaseController
{
    ...
    public function exportList(Request $request, $status): StreamedResponse|string|RedirectResponse
    {
        ...
        foreach ($orders as $item) {
            if(in_array($item->order_status, ['confirmed', 'delivered', 'processing', 'failed', 'out_for_delivery', 'returned'])){
                continue;
            }
    ...
    ): JsonResponse
    {
        ...
        if ($order['delivery_man_id'] && $request->order_status == 'delivered') {
            ...
            $cashInHand = $order['payment_method'] == 'pay_on_delivery' ? $order['order_amount'] : 0;
