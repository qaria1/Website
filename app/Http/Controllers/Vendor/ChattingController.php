...
class ChattingController extends BaseController
{
    ...
    public function getListView(string|array $type):View
    {
        ...
        if ($type == 'delivery-man') {
            ...
            if (isset($lastChat)) {
                ...
                return view(Chatting::INDEX[VIEW], compact('chattings', 'chattingUser', 'lastChat', 'shop','userType'));
            }
            ...
            $lastChat = $this->chattingRepo->getFirstWhereNotNull(
                params: ['seller_id' =>$vendorId],
                filters: ['admin_id', 'seller_id'],
                orderBy: ['created_at' => 'DESC']
            );

            if (isset($lastChat)) {
                $this->chattingRepo->updateAllWhere(
                    params: ['seller_id' => $vendorId, 'admin_id' => $lastChat['admin_id']],
                    data: ['seen_by_seller' => 1]);

                $chattings = $this->getChatList(
                    tableName: 'admins',
                    orderBy : 'desc',
                    id: $lastChat['admin_id'],
                );
    ...
    public function addVendorMessage(ChattingRequest $request):JsonResponse
    {
        ...
        if ($request->has(key: 'delivery_man_id')) {
            ...
            $admin = $this->adminRepo->getFirstWhere(params: ['id' => $request['admin_id']]);
            ChattingEvent::dispatch('message_from_seller', 'admin', $admin, $vendor);
        }
        $imageArray = [];
    ...
    protected function getChatList(string $tableName, string $orderBy, string|int $id = null) :Collection
    {
        ...
        if($tableName == 'users'){
            $columnName = 'user_id';
        }elseif($tableName == 'admins'){
            $columnName = 'admin_id';
        }else{
            $columnName = 'delivery_man_id';
        }
        ...
        return $this->chattingRepo->getListBySelectWhere(
            ...
            select: [
                'chattings.*',
                $tableName == 'admins' ? $tableName . '.name' : $tableName . '.f_name',
                $tableName == 'admins' ? $tableName . '.name' : $tableName . '.l_name',
                $tableName == 'admins' ? $tableName . '.image' : $tableName . '.image'],
            filters: $filters,
