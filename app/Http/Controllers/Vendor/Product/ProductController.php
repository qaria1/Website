...
class ProductController extends BaseController
{
    ...
    public function getListView(Request $request, $type): View
    {
        ...
        if(isset($type)){
            ProductModel::where([
                'request_status' => $type == 'approved'  ? '1' : '2',
                'checked_by_seller' => 0,
                'added_by' => 'seller',
                'user_id' => auth('seller')->id()
            ])->update([
                'checked_by_seller' => true,
            ]);
        }
    ...
    public function getAddView(): View
    {
        ...
        return view(Product::ADD[VIEW], compact('languages', 'categories', 'brands', 'brandSetting', 'digitalProductSetting', 'colors', 'attributes', 'languages', 'defaultLanguage', 'seller'));
    }

    public function getSellersLatestSubscriptionPlans($sellerId)
    {
        // get seller with id 1
        // $seller = Seller::find($sellerId)->id;
        $sellerSubscription = SellerSubscription::where('seller_id', $sellerId)->where('status', true)->orderBy('id', 'desc')->first();
        $latestSubscriptionPlanName = $sellerSubscription?->plan?->code;
        return $latestSubscriptionPlanName;
    }
    ...
    public function getUpdateView(string|int $id): RedirectResponse|View
    {
        ...
        return view(Product::UPDATE[VIEW], compact('product', 'categories', 'brands', 'brandSetting', 'digitalProductSetting', 'colors', 'attributes', 'languages', 'defaultLanguage', 'seller'));
    }
        ...
        foreach ($products as $item) {
            ...
            foreach (json_decode($item->category_ids, true) as $category) {
                if ($category['position'] == 1) {
                    ...
                    $category_id = \App\Models\Category::find($category['id'])?->name;
                } else if ($category['position'] == 2) {
                    // $sub_category_id = $category['id'];
                    $sub_category_id =  \App\Models\Category::find($category['id'])?->name;
                } else if ($category['position'] == 3) {
                    // $sub_sub_category_id = $category['id'];
                    $sub_sub_category_id =  \App\Models\Category::find($category['id'])?->name;
                }
            }
            $storage[] = [
                'name' => $item->name,
