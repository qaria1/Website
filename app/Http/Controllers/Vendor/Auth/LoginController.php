    ...
    public function login(LoginRequest $request): RedirectResponse
    {
        ...
        if (isset($vendor) && $vendor['status'] !== 'approved') {
            $statusMessages = [
                ...
                'suspended' => [
                    'reason' => translate('reason') . ': ' . $vendor->suspend_reason,
                    'title' => translate('account_suspended'),
                ],
                'rejected' => [
                    'reason' => translate('reason') . ': ' . $vendor->reject_reason,
                    'title' => translate('account_rejected'),
                ],
            ];
        ...
        return redirect()->back()
            ->withInput($request->only('email', 'remember'))
            ->withErrors([translate('invalid_credentials')]);
    }
