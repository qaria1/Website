...
class ProductController extends Controller
{
    ...
    public function submit_product_review(Request $request)
    {
        ...
        //         $loyaltyPointReviewItemPoint = getWebConfig(name:'loyalty_point_review_item_point');
        //         $user->loyalty_point = $user->loyalty_point !== NULL ? ($user->loyalty_point + $loyaltyPointReviewItemPoint) : $loyaltyPointReviewItemPoint;
        //         $user->save();
        //     }
        // }
        ...
        $alreadyReviewed = Review::where(['product_id' => $request->product_id, 'customer_id' => $request->user()->id])->count() > 0;
        if ($loyaltyPointStatus == 1 && isset($user) && !$alreadyReviewed) {
            $this->loyaltyPointTransactionRepo->addLoyaltyPointTransaction(userId: $user->id, reference: 'review_item_reference', amount: $loyaltyPointReviewItemPoint, transactionType: 'review_item');
        }
