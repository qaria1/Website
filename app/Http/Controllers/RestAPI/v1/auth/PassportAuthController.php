...
class PassportAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $user = User::create([
            ...
            // 'loyalty_point' => $loyaltyPointBecomeMemberPoint,
        ...
        // Loyalty point update for the referrer user
        if(isset($refer_user)){
            if($loyaltyPointStatus == 1){

                $loyaltyPointReferUserPoint = getWebConfig(name:'loyalty_point_refer_user_point');
                $this->loyaltyPointTransactionRepo->addLoyaltyPointTransaction(userId: $refer_user->id, reference: 'refer_user_reference', amount: $loyaltyPointReferUserPoint, transactionType: 'refer_user');
            }
        }

        if ($loyaltyPointStatus == 1 && isset($user)) {
            $loyaltyPointBecomeMemberPoint = getWebConfig(name:'loyalty_point_become_member_point');
            $this->loyaltyPointTransactionRepo->addLoyaltyPointTransaction(userId: $user->id, reference: 'register_reference', amount: $loyaltyPointBecomeMemberPoint, transactionType: 'become_member');
        }
    ...
    public function login(Request $request)
    {
        ...
        if (isset($user)) {
            ...
            if($user->is_active && auth()->attempt($data)){
                ...
                if ($lastLogin && Carbon::parse($lastLogin)->lessThan($twenty_four_hours_ago)) {

                    $loyaltyPointStatus = getWebConfig(name: 'loyalty_point_status');
                    // if($loyaltyPointStatus == 1){

                    //     $loyaltyPointVisitPlatformPoint = getWebConfig(name:'loyalty_point_visit_platform_point');
                    //     $user->loyalty_point = $user->loyalty_point !== NULL ? ($user->loyalty_point + $loyaltyPointVisitPlatformPoint) : $loyaltyPointVisitPlatformPoint;
                    // }

                    $loyaltyPointVisitPlatformPoint = getWebConfig(name:'loyalty_point_visit_platform_point');

                    if ($loyaltyPointStatus == 1 && isset($user)) {
                        $this->loyaltyPointTransactionRepo->addLoyaltyPointTransaction(userId: $user->id, reference: 'platform_visit_reference', amount: $loyaltyPointVisitPlatformPoint, transactionType: 'platform_visit');
                    }
                }
    ...
    {
        $lastLoginUpdate = !empty(auth()->user()) ? User::where('id', auth()->user()->id)->update([
            'last_login' => now(),
        ]) : '';
