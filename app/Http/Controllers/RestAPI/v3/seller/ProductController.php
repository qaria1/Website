...
class ProductController extends Controller
{
    ...
    public function get_seller_all_products($seller_id, Request $request)
    {
        ...
        $sellerActivePlan = $seller?->getActivePlanForSeller($seller);
        $sellerLeftProductCount = (int)$seller?->getDefaultProductCountForPlan($seller) - (int)$seller->getTotalProductForActivePlan($seller);
        ...
        return [
            ...
            'seller_active_plan' => $sellerActivePlan,
            'seller_product_count' => $sellerLeftProductCount
        ];
    }
    ...
    public function add_new(Request $request)
    {
        ...
        $sellerData = \App\Models\Seller::find($seller->id);
        $activePlanRecord = $sellerData?->getActivePlanForSeller($sellerData);
        ...
        $product->lifetime_end_date = $lifetimeEndDate;

        // product code generation based on subscription packages
        $seller_subscription = \App\Models\SellerSubscription::where('seller_id', $seller->id)->with(['plan'])->latest()->first();
        $latestSubscriptionPlanName = $seller_subscription?->plan?->code ?? '';

        $categoryCode = intval($request->category_id);
        $subCategoryCode = !empty($request->sub_category_id) ? intval($request->sub_category_id) : null;

        $categoryCode = (Category::find($categoryCode))?->category_code;
        $subCategoryCode = !empty($request->sub_category_id) ? Category::find($subCategoryCode)?->category_code : '';
        $deliveryClassCode =
        !empty($request->sub_category_id) ?
        Category::where('id', intval($request->sub_category_id))->first()?->deliveryClass?->code :
        Category::where('id', intval($request->category_id))->first()?->deliveryClass?->code;

        // Get the last product_sub_code with the given category and subcategory codes
        $lastProductSubCode = Product::where('product_sub_code', 'like', $latestSubscriptionPlanName . $categoryCode . $subCategoryCode . $deliveryClassCode . '%')
        ...
        $product->product_sub_code = $productSubCode;
        $product->seller_subscription_id = $seller_subscription?->id ?? 0;
    ...
    public function get_categories(Request $request)
    {
        ...
        $sellerActivePlan = $seller?->getActivePlanForSeller($seller);
        $sellerLeftProductCount = isset($sellerActivePlan) ? (int)$sellerActivePlan?->max_product_upload - (int)$seller?->getTotalProductForActivePlan($seller) : null;
        $sellerLeftProductCount = isset($sellerLeftProductCount) && $sellerLeftProductCount !== 0 ? true : false;

        $data = array();
        $data['has_seller_active_plan'] =  $sellerActivePlan != null ? true : false;
