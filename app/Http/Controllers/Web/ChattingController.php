...
class ChattingController extends Controller
{
    ...
    public function chat_list(Request $request, $type)
    {
        ...
        elseif ($type == 'admin') {
            $last_chat = Chatting::with(['admin'])->where('user_id', auth('customer')->id())
            ...
            if (isset($last_chat)) {
                // theme_aster - specific shop start
                if ($request->type =='admin' || $request->has('id') ){
                    $last_chat = Chatting::with(['shop'])->where(['user_id'=> auth('customer')->id(),'admin_id'=> 0])
                            ->whereNotNull(['admin_id', 'user_id'])
                            ->orderBy('created_at', 'DESC')
                            ->first();
                        Chatting::with(['shop'])->where(['user_id'=> auth('customer')->id(),'admin_id'=> 0])
                            ->whereNotNull(['admin_id', 'user_id'])
                            ->orderBy('created_at', 'DESC')
                            ->update(['seen_by_customer' =>1]);
                }
                ...
                $unique_shops = Chatting::join('admins', 'admins.id', '=', 'chattings.admin_id')
                    ->select('chattings.*', 'admins.name', 'admins.image')
                    ->where('chattings.user_id', auth('customer')->id())
                    ->when(theme_root_path()=='default' ,function($query){
                        return $query->orderBy('chattings.created_at', 'desc');
