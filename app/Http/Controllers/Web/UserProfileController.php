...
class UserProfileController extends Controller
{
    ...
    public function user_update(Request $request)
    {
        ...
        $user = User::where('id', auth('customer')->id())->first();
        // if(isset($user) && $user->is_profile_completed !== 1){

        //     if($loyaltyPointStatus == 1){

        //         $loyaltyPointCompleteProfilePoint = $loyaltyPointStatus == 1 ? getWebConfig(name:'loyalty_point_complete_profile_point') : NULL;
        //         User::where('id', auth('customer')->id())->update([
        //             'is_profile_completed' => 1,
        //             'loyalty_point' => $user->loyalty_point !== NULL ? ($user->loyalty_point + $loyaltyPointCompleteProfilePoint) : $loyaltyPointCompleteProfilePoint,
        //         ]);
        //     }

        // }

        $loyaltyPointCompleteProfilePoint = getWebConfig(name:'loyalty_point_complete_profile_point');

        if ($loyaltyPointStatus == 1 && isset($user) && $user->is_profile_completed != 1) {
            $this->loyaltyPointTransactionRepo->addLoyaltyPointTransaction(userId: $user->id, reference: 'complete_profile_reference', amount: $loyaltyPointCompleteProfilePoint, transactionType: 'complete_profile');
            User::where('id', auth('customer')->id())->update([
                'is_profile_completed' => 1,
            ]);
        }
    ...
    public function account_order_details(Request $request)
    {
        ...
        $order = $this->order->with([
            ...
            'customer',
            'offlinePayments',
            'details.product.reviewsByCustomer' => function ($query) {
        ...
        foreach ($order->details as $detail) {
            $orderDetails = json_decode($detail->product_details, true);
            $categoryId = $orderDetails['category_id'];
        ...
        // Group the categories by their IDs
        $groupedCategories = $categories->groupBy('id');

        // Retrieve categories matching the category IDs count
        $resultCategories = collect();
        foreach ($categoryIds as $categoryId) {
            $resultCategories = $resultCategories->merge($groupedCategories[$categoryId] ?? []);
        }

        $categories = $resultCategories;
        // dd($categories);      // $categories = Category::whereIn('id', $categoryIds)->get();


        $current_date = \Carbon\Carbon::now();
        if ($order) {
            return view(VIEW_FILE_NAMES['account_order_details'], compact('order', 'refund_day_limit', 'current_date', 'categories', 'allCategories'));
        }
    ...
    public function order_cancel($id)
    {
        ...
        if ($order['payment_method'] == 'pay_on_delivery' && $order['order_status'] == 'pending') {
