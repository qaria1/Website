...
class OrderManager
{
        ...
        if ($status == 'returned' || $status == 'failed' || $status == 'canceled') {
            foreach ($order->details as $detail) {
                if ($detail['is_stock_decreased'] == 1) {
                    ...
                    // what if the product is deleted? we should exlude those products here.
                    if(!$product){
                        continue;
                    }
    ...
    public static function wallet_manage_on_order_status_change($order, $received_by)
    {
        ...
        if ($order['payment_method'] == 'pay_on_delivery' || $order['payment_method'] == 'offline_payment') {
...
//        confirmed
        ...
        if ($or['payment_method'] != 'pay_on_delivery' && $or['payment_method'] != 'offline_payment') {
            $order_summary = OrderManager::order_summary($order);
