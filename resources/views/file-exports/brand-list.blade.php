    ...
    <table>
        <thead>
            ...
            <tr>
                ...
                <th>{{ translate('exclusive_Brand_Analytics').' '.'-' }}</th>
                <th></th>
                <th>
                        {{translate('total_Excluisve_Brand').' '.'-'.' '.count($data['brands'])}}
                    <br>
            ...
            @foreach ($data['brands'] as $key=>$item)
                ...
                <tr>
                    ...
                    <td> {{$item?->seller ? $item?->seller?->f_name.' '.$item?->seller?->l_name : translate("no_vendor")}}</td>
