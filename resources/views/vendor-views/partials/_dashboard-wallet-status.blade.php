    ...
    @if (!empty($dashboardData['totalEstimatedProfit']))
        <div class="col-lg-6">
            <!-- Card -->
            <div class="card h-100 d-flex justify-content-center align-items-center">
                <div class="card-body d-flex flex-column gap-10 align-items-center justify-content-center">
                    <img width="40" src="{{asset('/public/assets/back-end/img/ttg.png')}}" alt="">
                    <h3 class="for-card-count mb-0 fz-24">{{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $dashboardData['totalEstimatedProfit']), currencyCode: getCurrencyCode(type: 'default'))}}</h3>
                    <div class="font-weight-bold text-capitalize mb-30">
                        {{translate('total_estimated_profit')}}
                    </div>
                </div>
            </div>
        </div>
    ...
    @if (!empty($dashboardData['totalEarningProfit']))
        <div class="col-lg-6">
            <!-- Card -->
            <div class="card h-100 d-flex justify-content-center align-items-center">
                <div class="card-body d-flex flex-column gap-10 align-items-center justify-content-center">
                    <img width="40" src="{{asset('/public/assets/back-end/img/cc.png')}}" alt="">
                    <h3 class="for-card-count mb-0 fz-24">{{setCurrencySymbol(amount: usdToDefaultCurrency(amount: $dashboardData['totalEarningProfit']), currencyCode: getCurrencyCode(type: 'default'))}}</h3>
                    <div class="font-weight-bold text-capitalize mb-30">
                        {{translate('total_earned_profit')}}
                    </div>
                </div>
            </div>
        </div>
