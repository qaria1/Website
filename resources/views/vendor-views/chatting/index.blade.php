...
@section('content')
    <div class="content container-fluid">
                ...
                <div class="col-xl-3 col-lg-4 chatSel">
                    <div class="card card-body px-0 h-100">
                        ...
                        <div class="inbox_people">
                            ...
                            @if (Request::is('vendor/messages/index/admin') == false)
                                        ...
                                        <i class="tio-search search-icon" aria-hidden="true"></i>
                                            <input
                                                ...
                                                id="myInput" type="text" placeholder="
                                                    ...
                                                    @elseif(Request::is('vendor/messages/index/delivery-men'))
                                                        {{ translate('search_delivery_men') }}
                                                    @else
                                                        {{-- {{ translate('search_admin') }} --}}
                                                    @endif
                                                "
                            ...
                            <div class="inbox_chat d-flex flex-column mt-1">
                                @foreach($chattingUser as $key => $chatting)
                                    @if(isset($chatting->admin_id) && $chatting->admin)
                                        <div class="list_filter">
                                            <div class="chat_list p-3 d-flex gap-2 messageView user_{{$chatting->admin_id}} seller-list @if ($key == 0) active @endif"
                                                id="{{$chatting->admin_id}}" data-name="{{$chatting->f_name}} {{$chatting->l_name}}" data-phone="{{ $chatting->phone }}"
                                                data-image="{{ getValidImage(path: 'storage/app/public/admin/'.$chatting->admin->image,type: 'backend-profile') }}"
                                                data-user_id="{{ $chatting->admin_id }}"
                                            >
                                                <div class="chat_people media gap-10" id="chat_people">
                                                    <div class="chat_img avatar avatar-sm avatar-circle">
                                                        <img src="{{ getValidImage(path:'storage/app/public/admin/'.$chatting->admin->image,type: 'backend-profile') }}"
                                                            id="{{$chatting->admin_id}}" class="avatar-img avatar-circle" alt="">
                                                        <span class="avatar-satatus avatar-sm-status avatar-status-success"></span>
                                                    </div>
                                                    <div class="chat_ib media-body" style="width: initial !important;">
                                                        <h5 class="mb-1 seller {{$chatting->seen_by_seller ?'active-text' :''}}"
                                                            id="{{$chatting->admin_id}}" data-name="{{$chatting->name}}" data-phone="{{ $chatting->phone }}">
                                                            {{$chatting->name}}
                                                            <br><span class="mt-2 font-weight-normal text-muted" id="{{$chatting->admin_id}}" data-name="{{$chatting->name}}" data-phone="{{ $chatting->phone }}">{{ $chatting->phone }}</span>
                                                        </h5>
                                                    </div>
                                                </div>
                                                @if($chatting->seen_by_seller)
                                                    <div class="message-status bg-danger" id="notif-alert-{{ $chatting->admin_id }}"></div>
                                                @endif
                                            </div>
                                ...
                                <div class="avatar avatar-sm avatar-circle">
                                    <img class="avatar-img" id="profile_image"
                                         src="
                                            ...
                                            @if (Request::is('vendor/messages/index/customer'))
                                                {{ getValidImage(path:'storage/app/public/profile/'.$chattingUser[0]->image,type: 'backend-profile') }}
                                            @elseif(Request::is('vendor/messages/index/delivery-men'))
                                                {{ getValidImage(path:'storage/app/public/delivery-man/'.$chattingUser[0]->image,type: 'backend-profile') }}
                                            @else
                                                {{ getValidImage(path:'storage/app/public/admin/'.$chattingUser[0]->image,type: 'backend-profile') }}
                                            @endif
                                ...
                                <div class="input_msg_write">
                                    <form class="mt-4" id="myForm" enctype="multipart/form-data">
                                        ...
                                        <input type="hidden" id="hidden_value" value="
                                            ...
                                            @if ($lastChat->user_id)
                                                {{ $lastChat->user_id }}
                                            @elseif($lastChat->delivery_man_id)
                                                {{ $lastChat->delivery_man_id }}
                                            @else
                                                {{ $lastChat->admin_id }}
                                            @endif

                                            {{-- {{$lastChat->user_id ?? $lastChat->delivery_man_id}} --}}

                                        "

                                        name="{{ $userType == 'customer' ? 'user_id' : 'admin_id' }}">
                                        <div class="position-relative">
                                            ...
                                            <label class="py-0 px-3 d-flex align-items-center m-0 cursor-pointer position-absolute top-3">
                                                ...
                                                <input type="file" id="msgfilesValue" class="h-100 position-absolute w-100 " hidden multiple accept=".jpg, .jpeg">
                                            </label>
                                            @endif
        ...
        <span id="chatting-post-url" data-url="
            ...
            @elseif(Request::is('vendor/messages/index/delivery-man'))
