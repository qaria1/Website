                                ...
                                <div class="row gy-2 gx-2 align-items-center text-left">
                                    <div class="">
                                        <select class="form-control"
                                            name="status" id="">
                                            <option
                                                value="{{ request('status') != null ? request('status') : null }}"
                                                {{ old('status') == '' ? 'selected' : '' }} disabled>
                                                {{ translate('select_status') }}
                                            </option>
                                            <option {{ old('status') == '' ? 'selected' : '' }} value="">{{ translate('all') }}</option>
                                            <option {{ old('status') == 'active' ? 'selected' : '' }} value="active">{{ translate('active') }}</option>
                                            <option {{ old('status') == 'expired' ? 'selected' : '' }} value="expired">{{ translate('expired') }}</option>
                                        </select>
                        ...
                        <table id="datatable"
                            ...
                            @foreach($coupons as $k=>$coupon)
                                <tr>
                                    ...
                                    <td>
                                        @if(Carbon\Carbon::parse($coupon['expire_date'])->endOfDay()->isPast())
