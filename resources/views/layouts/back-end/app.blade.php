...
<body class="footer-offset">
    ...
    <span id="get-root-path-for-toggle-modal-image"
        data-path="{{ asset('/public/assets/back-end/img/modal') }}"></span>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.notification_data_new_count').addClass('d-none');

            function fetchNotifications() {
                $.ajax({
                    url: '{{ route("admin.request-notification.index") }}',
                    type: 'GET',
                    success: function (data) {
                        var totalNotifications = data.pendingSignups + data.newProductRequests + data.newRefundRequests;
                        $('#notificationCount').text(totalNotifications);
                        if (totalNotifications > 0) {
                            $('#notification-list').addClass('blink');
                            $('.notification_data_new_count').removeClass('d-none');
                            $('.notification_data_new_count').text(totalNotifications);

                            var toastMessage = '{{ translate("you_have_new_notification_please_check") }}';
                            toastr.options.positionClass = 'toast-bottom-right'; // Set toastr position
                            toastr.success(toastMessage);
                            updateDropdown(data);
                        } else {
                            $('#notification-list').removeClass('blink');
                            $('.notification_data_new_count').addClass('d-none');
            ...
            function updateDropdown(data) {
                var dropdownContent = '';
                if (data.pendingSignups > 0) {
                    dropdownContent += '<a class="dropdown-item" href="{{ route("admin.sellers.sign-up-request") }}">{{ translate("vendor_Sign_Up_Request") }} <span class="badge-soft-danger float-right small py-1 px-2 rounded">' + data.pendingSignups + '</span></a><div class="dropdown-divider"></div>';

                }
                if (data.newProductRequests > 0) {
                    dropdownContent += '<a class="dropdown-item" href="{{ route("admin.products.list", ["seller", "status" => "0"]) }}">{{ translate("vendor_Product_Request") }} <span class="badge-soft-danger float-right small py-1 px-2 rounded">' + data.newProductRequests + '</span></a><div class="dropdown-divider"></div>';
                }

                if (data.newRefundRequests > 0) {
                    dropdownContent += '<a class="dropdown-item" href="{{ route("admin.refund-section.refund.list", ["pending"]) }}">{{ translate("customer_Refund_Request") }} <span class="badge-soft-danger float-right small py-1 px-2 rounded">' + data.newRefundRequests + '</span></a>';
                }

                if(data.newProductRequests == 0 && data.pendingSignups == 0 && data.newRefundRequests == 0){
                    dropdownContent += '<span class="dropdown-item">{{ translate("no_Notification_Found!") }}</span>';
                }
