            ...
            <div class="navbar-nav-wrap-content-right"
                ...
                <ul class="navbar-nav align-items-center flex-row">
                    ...
                    <li class="nav-item d-none d-md-inline-block">
                        <div class="hs-unfold">
                            <a
                                class="border js-hs-unfold-invoker btn btn-icon btn-ghost-secondary media align-items-center gap-3 navbar-dropdown-account-wrapper dropdown-toggle-left-arrow dropdown-toggle-empty"
                                href="javascript:"
                                data-hs-unfold-options='{
                                     "target": "#notificationDropdown",
                                     "type": "css-animation"
                                   }'>
                                   <span class="px-3 py-2" id="notification-list">
                                    <i class="tio-notifications-on-outlined"></i>
                                   </span>
                                    <span id="bell-badge"
                                        class="btn-status btn-sm-status btn-status-danger notification_data_new_count d-none"></span>
                            </a>
                            ...
                            <div class="hs-unfold">
                                <a class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle"
                                   ...
                                   >
                                    ...
                                    {{-- @php($inboxMessage=\App\Models\Chatting::where('admin_id', 0)->where('seen_by_admin', 0)->where(function($query){ $query->where('sent_by_seller', 1)->orWhere('sent_by_customer', 1)})->count()) --}}

                                    <?php
                                        $inboxMessage = \App\Models\Chatting::where('admin_id', 0)->where('seen_by_admin', 0)
                                        ->where(function($query){
                                            $query->where('sent_by_seller', 1)
                                                ->orWhere('sent_by_customer', 1);
                                        })->count();
                                    ?>

                                    @if($inboxMessage!=0)
                                        <span class="btn-status btn-sm-status btn-status-danger">{{ $inboxMessage }}</span>
                                    @endif
                                </a>
                            </div>
                        </li>
