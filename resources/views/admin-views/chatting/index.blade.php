...
@section('content')
    <div class="content container-fluid">
                                ...
                                <div class="inbox_chat d-flex flex-column">
                                    @if(isset($allChattingUsers) && count($allChattingUsers) > 0)
                                        @foreach($allChattingUsers as $key => $chatting)
                                            @if(isset($chatting->user_id) && $chatting->customer)
                                            ...
                                            @elseif($chatting->seller_id && $chatting->sellerInfo)
                                                <div class="list_filter">
                                                    <div
                                                        class="chat_list p-3 d-flex gap-2 @if ($key == 0) bg-soft-secondary @endif get-ajax-message-view"
                                                        ...
                                                        <div class="chat_people media gap-10 w-100" id="chat_people">
                                                            <div class="chat_img avatar avatar-sm avatar-circle">
                                                                <img
                                                                    src="{{ getValidImage(path:'storage/app/public/seller/'.$chatting->sellerInfo->image,type: 'backend-profile') }}"
                                                            ...
                                                            <div class="chat_ib media-body">
                                                                <h5 class="mb-1 seller {{$chatting->seen_by_seller ?'active-text' :''}}"
                                                                    id="{{ $chatting->seller_id }}"
                                                                    data-name="{{ $chatting->sellerInfo->f_name.' '.$chatting->sellerInfo->l_name }}"
                                                                    data-phone="{{ $chatting->sellerInfo->phone }}">
                                                                    {{ $chatting->sellerInfo->f_name.' '.$chatting->sellerInfo->l_name }}
                                                                ...
                                                                <span
                                                                    ...
                                                                    id="{{ $chatting->seller_id }}"
                                                                    data-name="{{ $chatting->sellerInfo->f_name .' '. $chatting->sellerInfo->l_name}}"
                                                                    data-phone="{{ $chatting->sellerInfo->phone }}">{{ $chatting->sellerInfo->phone }}</span>
                                                            </div>
                                                        </div>
                                                        @if(!$chatting->seen_by_admin && !($key == 0))
                                                            <div
                        ...
                        <div class="inbox_msg_header d-flex flex-wrap gap-3 justify-content-between align-items-center border px-3 py-2 rounded mb-4">
                            <div class="media align-items-center gap-3">
                                    ...
                                    <img class="avatar-img user-avatar-image" id="profile_image"
                                         src="
                                            ...
                                            @if(request('type') == 'customer')
                                                {{ getValidImage(path: 'storage/app/public/profile/'.$lastChatUser['image'],type: 'backend-profile') }}
                                            @elseif(request('type') == 'delivery_man')
                                                {{ getValidImage(path:'storage/app/public/delivery-man/'.$lastChatUser['image'],type: 'backend-profile') }}
                                            @else
                                                {{ getValidImage(path:'storage/app/public/seller/'.$lastChatUser['image'],type: 'backend-profile') }}
                                            @endif
                                ...
                                <div class="media-body">
                                    ...
                                    <span class="fz-12" id="profile_phone">{{ $lastChatUser['phone'] }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="type_msg">
                            <div class="input_msg_write">
                                <form class="mt-4 chatting-messages-ajax-form" enctype="multipart/form-data">
                                    @csrf
                                    <input type="hidden" id="current-user-hidden-id" value="{{ $lastChatUser->id }}" name="{{ $userType == 'customer' ? 'user_id' : 'seller_id' }}">
        ...
        <span id="chatting-post-url"
              data-url="
                    @if (Request::is('admin/messages/index/customer'))
                        {{ route('admin.messages.message').'?user_id=' }}
                    @elseif(Request::is('admin/messages/index/delivery-man'))
