                                ...
                                <div class="col-sm-6 col-12">
                                    <div class="mb-3">
                                        ...
                                        <select name="transaction_type" id="" class="form-control"
                                            ...
                                            <option value="become_member"
                                                {{ isset($transaction_status) && $transaction_status == 'become_member' ? 'selected' : '' }}>
                                                {{ translate('become_member') }}</option>
                                            <option value="platform_visit"
                                                {{ isset($transaction_status) && $transaction_status == 'platform_visit' ? 'selected' : '' }}>
                                                {{ translate('platform_visit') }}</option>
                                            <option value="refer_user"
                                                {{ isset($transaction_status) && $transaction_status == 'refer_user' ? 'selected' : '' }}>
                                                {{ translate('refer_user') }}</option>
                                            {{-- <option value="social_media" {{isset($transaction_status) && $transaction_status=='social_media'?'selected':''}}>{{ translate('social_media')}}</option> --}}
                                            <option value="review_item"
                                                {{ isset($transaction_status) && $transaction_status == 'review_item' ? 'selected' : '' }}>
                                                {{ translate('review_item') }}</option>
                                            <option value="complete_profile"
                                                {{ isset($transaction_status) && $transaction_status == 'complete_profile' ? 'selected' : '' }}>
                                                {{ translate('complete_profile') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-6 col-12">
                                ...
                                <div class="col-sm-6 col-12">
                                    <div class="mb-3">
                                        ...
                                        <select name="sort_by" id="" class="form-control"
                                            ...
                                            <option value="loyalty_point_asc"
                                                {{ isset($sort_by) && $sort_by == 'loyalty_point_asc' ? 'selected' : '' }}>
                                                {{ translate('loyalty_point_asc') }}</option>
                                                <option value="loyalty_point_desc"
                                                {{ isset($sort_by) && $sort_by == 'loyalty_point_desc' ? 'selected' : '' }}>
                                                {{ translate('loyalty_point_desc') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                    ...
                    <tbody>
                        @foreach ($transactions as $key => $transaction)
                            <tr scope="row">
                                ...
                                <td><a href="{{ route('admin.customer.view', ['user_id' => $transaction['user_id']]) }}"
                                        class="title-color hover-c1">{{ Str::limit($transaction->user ? $transaction->user->f_name . ' ' . $transaction->user->l_name : translate('not_found'), 20) }}</a>
                                </td>
                                ...
                                <td class="text-center">
                                    {{ date('Y/m/d ' . config('timeformat'), strtotime($transaction['created_at'])) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
