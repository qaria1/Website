    ...
    <div class="content container-fluid">
        ...
        <div class="card mt-3">
            ...
            <div class="table-responsive pb-3">
                <table
                    class="table table-hover table-condensed table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                            ...
                            <th>{{ translate('price_on_billing') }}</th>
                    ...
                    <tbody>
                        @foreach ($subscriptionPlans as $key => $plan)
                            <tr>
                                ...
                                <td>
                                    @if ($plan->billingTypes->count() > 0)
                                        @foreach ($plan->billingTypes as $billing)
                                            <p>{{ $billing->name }}:
                                                <strong>
                                                    {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $billing->planBillings()->where('plan_id', $plan->id)->first()->price),currencyCode: getCurrencyCode(type: 'default')) }}
                                                </strong>
                                            </p>
                                        @endforeach
                                    @else
                                        <p class="badge badge-warning">{{ translate('no_billing_record') }}!</p>
                                    @endif
                                ...
                                <td>
                                    <div class="d-flex flex-wrap justify-content-center gap-10">
                                        <a class="btn btn-outline--primary btn-sm edit" title="{{ translate('edit') }}"
                                            href="{{ route('admin.business-settings.subscription.update', [$plan->id]) }}">
                                            <i class="tio-edit"></i>
                                        </a>
                                        <a class="btn btn-outline--primary btn-sm edit"
                                            title="{{ translate('add_billing_type') }}"
