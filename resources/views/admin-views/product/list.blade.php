                        ...
                        <table id="datatable"
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th class="text-right">{{ translate('product_Type') }}</th>
                                    <th class="text-right">{{ translate('product_Code') }}</th>
                            ...
                            <tbody>
                                @foreach ($products as $key => $product)
                                    <tr>
                                        ...
                                        <td class="text-right">
                                            {{ translate(str_replace('_', ' ', $product['product_sub_code'])) }}
                                        </td>
                                        <td class="text-right">
                                            ...
                                            <form action="{{ route('admin.products.featured-status') }}" method="post"
                                                ...
                                                <label class="switcher mx-auto">
                                                    <input type="checkbox" class="switcher_input toggle-switch-message"
                                                        name="status" id="product-featured{{ $product['id'] }}"
                                            ...
                                            @if (request('type') == 'seller')
                                                @if (!$isProductArchived)
                                                    <form action="{{ route('admin.products.status-update') }}" method="post"
                                                        data-from="product-status" id="product-status{{ $product['id'] }}-form"
                                                        class="admin-product-status-form">
                                                        @csrf
                                                        <input type="hidden" name="id" value="{{ $product['id'] }}">
                                                        <label class="switcher mx-auto">
                                                            <input type="checkbox" class="switcher_input toggle-switch-message"
                                                                name="status" id="product-status{{ $product['id'] }}"
                                                ...
                                                @else
                                                    <span class="badge badge-soft-danger" title="{{ translate('sorry_you_can_not_change_status_of_product.') }}">{{ translate('product_archived') }}</span>
                                                @endif
                                            @else
                                                <form action="{{ route('admin.products.status-update') }}" method="post"
                                                    data-from="product-status" id="product-status{{ $product['id'] }}-form"
