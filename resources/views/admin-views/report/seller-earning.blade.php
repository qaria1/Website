    ...
    <div class="content container-fluid">
        ...
        <div class="store-report-content mb-2">
            <div class="left-content">
                ...
                @if (!empty(\App\Models\Seller::getTotalVendorEstimatedProfit()))
                    <div class="left-content-card">
                        <img width="40" src="{{asset('/public/assets/back-end/img/ttg.png')}}" alt="">
                        <div class="info">
                            <h4 class="subtitle">{{ \App\Models\Seller::getTotalVendorEstimatedProfit() ? setCurrencySymbol(amount: usdToDefaultCurrency(amount: \App\Models\Seller::getTotalVendorEstimatedProfit()), currencyCode: getCurrencyCode()) : 0 }}</h4>
                            <h6 class="subtext">{{ translate('total_Estimated_Profit')}}</h6>
                        </div>
                    </div>
                @endif
                @if (!empty(\App\Models\Seller::getTotalVendorEarningProfit()))
                    <div class="left-content-card">
                        <img width="40" src="{{asset('/public/assets/back-end/img/cc.png')}}" alt="">
                        <div class="info">
                            <h4 class="subtitle">{{ \App\Models\Seller::getTotalVendorEarningProfit() ? setCurrencySymbol(amount: usdToDefaultCurrency(amount: \App\Models\Seller::getTotalVendorEarningProfit()), currencyCode: getCurrencyCode()) : 0 }}</h4>
                            <h6 class="subtext">{{ translate('total_Earned_Profit')}}</h6>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="table-responsive">
                <table id="datatable"
                    ...
                    <tr>
                        ...
                        <th>{{translate('estimated_Profit')}}</th>
                        <th>{{translate('earned_Profit')}}</th>
                    </tr>
                    </thead>
                    ...
                    @foreach($table_earning['seller_earn_table'] as $key=>$seller_earn)
                        ...
                        <tr>
                            ...
                            <td>
                                @php($seller = \App\Models\Seller::find($seller_earn['seller_id']))
                                @php($sellerEstimatedProfit = $seller?->getVendorEstimatedProfit($seller))
                                @if (!empty($sellerEstimatedProfit))
                                    {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $sellerEstimatedProfit), currencyCode: getCurrencyCode()) }}
                                @else
                                    <span class="badge badge-soft-warning">{{ translate('purchase_price_not_set') }}</span>
                                @endif
                            </td>
                            <td>
                                @php($seller = \App\Models\Seller::find($seller_earn['seller_id']))
                                @php($sellerEarnedProfit = $seller?->getVendorEarningProfit($seller))
                                @if (!empty($sellerEarnedProfit))
                                {{ setCurrencySymbol(amount: usdToDefaultCurrency(amount: $sellerEarnedProfit), currencyCode: getCurrencyCode()) }}
                                @else
                                    <span class="badge badge-soft-warning">{{ translate('purchase_price_not_set') }}</span>
                                @endif
                            </td>
