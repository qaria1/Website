...
@section('title', translate('billing_type'))
...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="card mt-3">
            <div class="border-bottom px-4 py-3">
                <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2 text-capitalize">
                    {{ translate('subscription_billing_type_setting') }}
                </h5>
            </div>
            <div class="table-responsive pb-3">
                <table
                    ...
                    <tbody>
                        @foreach ($billingTypes as $key => $billing)
                            <tr>
                                <th>{{ $key + 1 }}</th>
                                <td>{{ $billing->name ?? '-' }}</td>
                                {{-- <td>
        ...
        <div class="card mt-3">
            <div class="border-bottom px-4 py-3">
                <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2 text-capitalize">
                    {{ translate('add_new_billing_type') }}
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.business-settings.billing-type.add') }}" method="post"
                    enctype="multipart/form-data" id="add_billing_type">
                    @csrf
                    <div class="row align-items-end">
                        <div class="col-xl-4 col-md-6">
                            <div class="">
                                <label for="billingType">{{ translate('billing_name') }}</label>
                                <input type="text" name="billing_name" id="billingType"
                                    placeholder="{{ translate('Ex') }}: {{ translate('Monthly') }}" class="form-control">
                            </div>
                        </div>
                        <div class="col-xl-4 col-md-6">
                            <div class="">
                                <label for="billingDuration">
                                ...
                                <input required name="billing_duration" id="billingDuration" class="form-control"
                                    type="number" placeholder="{{ translate('duration_in_days') }}">
                            </div>
                        </div>
                    </div>
...
@push('script')
    <script src="{{ asset('public/assets/back-end/js/admin/shipping-method.js') }}"></script>
@endpush
