                        ...
                        <form action="{{ route('admin.category.update', [$category['id']]) }}" method="POST"
                            ...
                            <div class="row">
                                <div
                                    ...
                                    @foreach ($languages as $lang)
                                        <div>
                                            ...
                                            <div class="form-group {{ $lang != $defaultLanguage ? 'd-none' : '' }} form-system-language-form"
                                                id="{{ $lang }}-form">
                                                <label class="title-color">
                                                    {{ translate('category_Code') }} ({{ strtoupper($lang) }})
                                                </label>
                                                <input type="text" name="category_code[]"
                                                    value="{{ $lang == $defaultLanguage ? $category['category_code'] : $translate[$lang]['category_code'] ?? '' }}"
                                                    class="form-control" placeholder="{{ translate('new_Category_code') }}"
                                                    {{ $lang == $defaultLanguage ? 'required' : '' }}>
                                            </div>
                                        ...
                                        <select id="" name="delivery_class_id" class="form-control">
                                            ...
                                            @foreach ($deliveryClasses as $deliveryClass)
                                                <option value="{{ $deliveryClass['id'] }}"
                                                    {{ $category->deliveryClass?->id === $deliveryClass['id'] ? 'selected' : '' }}>
                                                    {{ $deliveryClass['name'] }}
                                                </option>
                                            @endforeach
                                    ...
                                    <div class="form-group w-100">
                                        <label class="title-color"
                                            for="exampleFormControlSelect2">{{ translate('refund_number_of_days') }} <span
                                                class="text-danger">*</span></label>
                                        <select id="" name="number_of_days" class="form-control">
                                            <option value="" selected disabled>
                                                {{ translate('select_refund_number_of_days') }}</option>
                                            @foreach ($categoryRefundDate as $refundDate)
                                                <option value="{{ $refundDate->id }}"
                                                    {{ $category->refundDate->id === $refundDate->id ? 'selected' : '' }}>
                                                    {{ $refundDate->number_of_days }} Days
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
