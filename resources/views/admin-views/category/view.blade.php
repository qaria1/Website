                                    ...
                                    <div>
                                        @foreach ($languages as $lang)
                                            <div class="form-group {{ $lang != $defaultLanguage ? 'd-none' : '' }} form-system-language-form"
                                                ...
                                                <div class="form-group pt-4" id="{{ $lang }}-form">
                                                    <label class="title-color">{{ translate('category_Code') }}<span
                                                            class="text-danger">*</span> ({{ strtoupper($lang) }})</label>
                                                    <input type="text" name="category_code[]" class="form-control"
                                                        placeholder="{{ translate('new_Category_Code') }}"
                                                        {{ $lang == $defaultLanguage ? 'required' : '' }}>
                                                </div>
                                                ...
                                                <div class="form-group w-100">
                                                    <label class="title-color"
                                                        for="exampleFormControlSelect1">{{ translate('main_Delivery_class') }}
                                                        <span class="text-danger">*</span></label>
                                                    <select id="" name="delivery_class_id" class="form-control">
                                                        <option value="" selected disabled>
                                                            {{ translate('select_delivery_class') }}
                                                        </option>

                                                        @foreach ($deliveryClasses as $deliveryClass)
                                                            <option value="{{ $deliveryClass['id'] }}">
                                                                {{ $deliveryClass['name'] }}
                                                            </option>
                                                        @endforeach
                                                ...
                                                <div class="form-group w-100">
                                                    <label class="title-color"
                                                        for="exampleFormControlSelect1">{{ translate('refund_number_of_days') }}
                                                        <span class="text-danger">*</span></label>
                                                    <select id="" name="number_of_days" class="form-control">
                                                        <option value="" selected disabled>
                                                            {{ translate('select_number_of_days') }}
                                                        </option>

                                                        @foreach ($categoryRefundDate as $categoryRefundDat)
                                                            <option value="{{ $categoryRefundDat['id'] }}">
                                                                {{ $categoryRefundDat['number_of_days'] }} Days
                                                            </option>
                                                        @endforeach
                        ...
                        <table
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th>{{ translate('category_code') }}</th>
                                    <th>{{ translate('delivery_class') }}</th>
                                    <th>{{ translate('refund_date') }}</th>
                            ...
                            <tbody>
                                @foreach ($categories as $key => $category)
                                    <tr>
                                        ...
                                        <td>{{ $category['defaultcode'] }}</td>
                                        <td>{{ $category->deliveryClass?->name }}</td>
                                        {{-- @dd($category) --}}
                                        <td>{{ $category->refundDate?->number_of_days }} Days</td>
                                            ...
                                            <form action="{{ route('admin.category.status') }}" method="post"
                                                ...
                                                <label class="switcher mx-auto">
                                                    <input type="checkbox" class="switcher_input toggle-switch-message"
                                                        name="home_status" id="category-status{{ $category['id'] }}"
                                                        value="1"
