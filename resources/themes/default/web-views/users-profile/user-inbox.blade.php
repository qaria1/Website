                                    ...
                                    @foreach($unique_shops as $key=>$shop)
                                        @php($type = '')
                                            @if ($shop->delivery_man_id)
                                                @php($type = 'delivery-man')
                                            @elseif(isset($shop->admin_id))
                                                @php($type = 'admin')
                                            @else
                                                @php($type = 'seller')
                                            @endif
                                        ...
                                        <div class="chat_list {{($last_chat->delivery_man_id==$unique_id || $last_chat->shop_id==$unique_id) ? 'active' : ''}} get-view-by-onclick"
                                            ...
                                            <div class="chat_people">
                                                <div class="chat_img">
                                                    ...
                                                    @elseif(isset($shop->admin_id))
                                                        <img alt="" class="__inline-14 __rounded-10 img-profile" style="height: auto;"
                                                             src="{{ getValidImage(path: 'storage/app/public/admin/'.$shop->image, type: 'avatar') }}">
                                                    @else
                                                        <img alt="" class="__inline-14 __rounded-10 img-profile"
                                        ...
                                        <div class="mesgs">
                                            <a class="msg-user" href="#">
                                                ...
                                                <h5 class="m-0">
                                                    ...
                                                    @if(isset($last_chat->admin))
                                                        {{ $last_chat->admin->name }}
                                                    @elseif($last_chat->deliveryMan)
                                            ...
                                            <div class=" msg_history d-flex flex-column-reverse" id="show_msg">
                                                @if (isset($chattings))
                                                    @foreach($chattings as $key => $chat)
                                                        ...
                                                        @if ($chat->sent_by_seller || $chat->sent_by_delivery_man || $chat->sent_by_admin)
                                                        ...
                                                        @else
                                                            <div class="outgoing_msg" style="width: initial !important;">
                                                                <div class="send_msg">
                                                                    @if($chat->message)
                                                                    <p class="btn--primary">
                                                                        {{$chat->message}}
            ...
            $("#msgSendBtn").click(function (e) {
                ...
                $.ajax({
                    ...
                    success: function (respons) {
                        ...
                        $(".msg_history").prepend(`
                            <div class="outgoing_msg" id="outgoing_msg" style="width: initial !important;">
                              <div class='send_msg'>
                                ${message}
                                ${imageContainer}
